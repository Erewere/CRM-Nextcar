...
export function UserSettingsModal({ isOpen, onClose }: UserSettingsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4">
      ...
      <motion.div 
        className="bg-white dark:bg-slate-900 md:rounded rounded-t-3xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col border border-gray-200 dark:border-slate-800 relative z-10"
        ...
        <div className="p-6">
          <div className="flex flex-col items-center mb-6">
            <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
              {userData.photoURL ? (
                ...
                <div className="w-20 h-20 rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 font-bold text-2xl flex items-center justify-center border-2 border-gray-200 dark:border-slate-700">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            {isMobile && userData.role === "admin" && (
              <div className="pt-2">
                <label className="flex items-center gap-3 cursor-pointer p-3 border border-gray-200 dark:border-slate-700 rounded hover:bg-[#f4f5f5] dark:hover:bg-slate-800/50 transition-colors">
            ...
            <div className="pt-4 flex gap-3">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 px-4 py-2 text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded font-medium transition-colors text-sm"
              >
                Cancelar
              </button>
              <button
                type="submit"
                disabled={isSaving}
                className="flex-1 px-4 py-2 text-white bg-blue-600 hover:bg-blue-700 rounded font-medium disabled:opacity-50 transition-colors text-sm"
              >
          ...
          <div className="mt-6 pt-6 border-t border-gray-200 dark:border-slate-800">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-2 text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-900/20 hover:bg-rose-100 dark:hover:bg-rose-900/40 rounded font-medium transition-colors text-sm"
            >
