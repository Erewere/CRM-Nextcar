...
export function KanbanColumn({ column, clients, onClientClick, tasks = [] }: Props) {
    ...
    <div className="flex w-[270px] flex-col h-full shrink-0 border-r border-slate-200 dark:border-slate-700/70 bg-[#F3F4F6] relative">
      ...
      <div 
        ...
        className={clsx(
          ...
          isOver ? "bg-blue-50/80 ring-2 ring-inset ring-blue-400 shadow-inner scale-[1.01]" : ""
        )}
      >
        <SortableContext 
          id={column.id} 
        ...
        >
          {clients.map(client => (
            <SortableClientCard key={client.id} client={client} onClick={() => onClientClick(client)} tasks={tasks.filter(t => t.clientId === client.id)} />
          ))}
        </SortableContext>
      </div>
    </div>
