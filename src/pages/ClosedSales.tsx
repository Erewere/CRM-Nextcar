  ...
  useEffect(() => {
    ...
    if (userData?.agencyId) {
      getDoc(doc(db, "agencies", userData.agencyId as string))
        ...
        .catch((err) => console.error("Error fetching agency pipeline stages:", err));
    }
    ...
    }, (err) => console.error("ClosedSales vehicles snapshot error:", err));
    ...
    }, (err) => console.error("ClosedSales deals snapshot error:", err));

    const unsubExpenses = onSnapshot(expensesQ, (snap) => {
      setExpenses(snap.docs.map(d => ({ ...d.data(), id: d.id } as VehicleExpense)));
      setLoading(false);
    }, (err) => {
      console.error("ClosedSales expenses snapshot error:", err);
      setLoading(false);
    });
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 bg-slate-100 dark:bg-slate-900 border-transparent rounded text-sm focus:bg-white dark:focus:bg-slate-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-900 w-full md:w-64 transition-all"
            />
          </div>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-[#f4f5f5] dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 uppercase text-xs font-semibold">
                <tr>
                  <th className="px-4 py-3 border-b border-gray-200 dark:border-slate-700">Fecha Venta</th>
                  <th className="px-4 py-3 border-b border-gray-200 dark:border-slate-700">Vehículo</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                {filteredSales.length === 0 ? (
                  ...
                  filteredSales.map((client, idx) => {
                    ...
                    return (
                      <tr key={`${client.id}-${idx}`} className="hover:bg-[#f4f5f5] dark:hover:bg-slate-800/50 transition-colors">
