...
# Replace the specific block of code using regex or substring
start_str = '  app.post("/api/meta/webhook", async (req, res) => {'
end_str = 'await addDoc(collection(adminDb, "clients"), newClient);\n  }'

if start_str in content and end_str in content:
    start_idx = content.find(start_str)
    end_idx = content.find(end_str) + len(end_str)
    
    new_webhook = """  app.post("/api/meta/webhook", async (req, res) => {
    const body = req.body;
  ...
  async function createMetaLead(
    ...
    content = content[:start_idx] + new_webhook + content[end_idx:]
