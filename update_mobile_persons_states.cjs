...
const stateStr = `  const [pipelineStages, setPipelineStages] = useState<PipelineStage[]>([]);
...
content = content.replace('  const [selectedClient, setSelectedClient] = useState<Client | null>(null);', '  const [selectedClient, setSelectedClient] = useState<Client | null>(null);\n' + stateStr);

const modalStr = `      {selectedClient && (
        <MobileClientDetail 
...
console.log('Updated state and modals');
